...
export default function Home() {
  ...
  return (
    <main>
      ...
      <section className="today-section page-shell" id="today"><div className="today-card"><div className="today-meta"><span className="section-index">TODAY / 01</span><span className="duration">约 15 分钟</span></div><div className="today-grid"><div><p className="card-label">今天不是只看一句</p><h2>先回忆，再打开答案。</h2><p className="translation">五项短任务，会根据你的表现进入之后的复习。</p><div className="phrase-tags"><span>听力</span><span>口语替换</span><span>句式</span></div></div><div className="today-action"><a className="primary-button" href={sitePath("/today")}>开始今日学习 <span>→</span></a><a className="text-action" href={sitePath("/review")}>查看复习安排 →</a></div></div></div><div className="stats-card"><div className="stats-top"><span>你的学习地图</span><b>{completed.length} / 04</b></div><div className="progress-track"><span style={{ width: `${Math.min(100, completed.length * 25)}%` }} /></div><div className="stats-list"><span><i className="dot coral-dot" />转换桥</span><span><i className="dot gold-dot" />生活场景</span><span><i className="dot mint-dot" />听力反应</span></div></div></section>
