  ...
  return (
    <main>
      <header className={`site-header${navOpen ? " nav-open" : ""}`}>
        ...
        <nav className="main-nav" id="main-nav-links" aria-label="主要导航"><a href="/today" onClick={() => setNavOpen(false)}>今日学习</a><a href="/bridge" onClick={() => setNavOpen(false)}>普通话 → 粤语</a><a href="/course" onClick={() => setNavOpen(false)}>情景课程</a><a href="/audio" onClick={() => setNavOpen(false)}>听力实验室</a><a href="/review" onClick={() => setNavOpen(false)}>复习</a></nav>
      ...
      <section className="today-section page-shell" id="today"><div className="today-card"><div className="today-meta"><span className="section-index">TODAY / 01</span><span className="duration">约 15 分钟</span></div><div className="today-grid"><div><p className="card-label">今天不是只看一句</p><h2>先回忆，再打开答案。</h2><p className="translation">五项短任务，会根据你的表现进入之后的复习。</p><div className="phrase-tags"><span>听力</span><span>口语替换</span><span>句式</span></div></div><div className="today-action"><a className="primary-button" href="/today">开始今日学习 <span>→</span></a><a className="text-action" href="/review">查看复习安排 →</a></div></div></div><div className="stats-card"><div className="stats-top"><span>你的学习地图</span><b>{completed.length} / 04</b></div><div className="progress-track"><span style={{ width: `${Math.min(100, completed.length * 25)}%` }} /></div><div className="stats-list"><span><i className="dot coral-dot" />转换桥</span><span><i className="dot gold-dot" />生活场景</span><span><i className="dot mint-dot" />听力反应</span></div></div></section>
      <div className="page-shell home-audio-settings"><CantoneseAudioSettings /></div>
      ...
      <section className="scenes-section" id="scenes"><div className="page-shell"><SectionIntro eyebrow="REAL LIFE / 真实场景" title="你不是为了背词，是为了把一件事办成。" copy="每个场景都从完整对话出发：开口、追问、处理意外，再自然结束互动。" /><div className="scene-grid">{scenes.map((scene) => <article className={`scene-card ${scene.color}`} key={scene.number}><div className="scene-head"><span>{scene.number}</span><small>{scene.label}</small></div><h3>{scene.title}</h3><p>{scene.copy}</p><div className="scene-phrase"><b>{scene.phrase}</b><small>{scene.jyutping}</small><SpeakButton text={scene.phrase} label={`播放：${scene.phrase}`} /></div><div className="scene-actions"><a className="scene-link" href={`/course/${scene.slug}`}>打开场景 →</a><button className="scene-complete" onClick={() => complete(`scene-${scene.number}`)} type="button">{completed.includes(`scene-${scene.number}`) ? "已加入学习 ✓" : "加入学习"}</button></div></article>)}</div></div></section>
